...
{ /* NVLINK */
  ...
  /* NET */
  { /* Tree (LL/LL128/Simple)*/ { 9.8, 9.8, 19.5 }, /* Ring (LL/LL128/Simple)*/ { 2.2, 2.2, 9.3 } }
};
  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      ...
      for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
        ...
        if (a == NCCL_ALGO_TREE) busBw = std::min(busBw*.9, comm->nNodes > 1 ? 2.3 : 80.0);
        if (a == NCCL_ALGO_TREE && p == NCCL_PROTO_LL) busBw *= 1.0/2.4;
