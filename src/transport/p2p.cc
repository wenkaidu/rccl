...
// cuMem API support
ncclResult_t ncclP2pAllocateShareableBuffer(size_t size, ncclIpcDesc *ipcDesc, void **ptr, hipStream_t stream) {
  if (ncclCuMemEnable()) {
    ...
    NCCLCHECK(ncclCudaCalloc((char **)ptr, size, stream, true));
...
static ncclResult_t p2pSendProxySetup(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  if (useMemcpy) {
    ...
    NCCLCHECK(ncclP2pAllocateShareableBuffer(size, &p2pBuff->ipcDesc, &p2pBuff->directPtr, nullptr));
    p2pBuff->size = size;
    if (ncclCuMemEnable()) {
      // cuMem API support
